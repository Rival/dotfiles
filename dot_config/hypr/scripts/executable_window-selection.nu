#!/usr/bin/env nu
def focus_window [] {
     let window_list = hyprctl clients -j | from json | each {|client|
         if $client.title != "" {
     ...
     let choice = $window_list | from json | select title | to text | cut -d ' ' -f 2- | wofi -dmenu -i -p "Select Window"
 ...
 $selected |print
         ...
         if $selected != null {
             echo "Focusing on window: $selected"
